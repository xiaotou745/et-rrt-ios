...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_modeArr.count > 0){
        ...
        cell.topLab.text = [NSString stringWithFormat:@"失效时间 %@",[task.dealLineTime substringToIndex:16]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    Task *task = [_modeArr objectAtIndex:indexPath.section];
    [[NSNotificationCenter defaultCenter]postNotificationName:@"myselect" object:task userInfo:@{@"type":@"6"}];
}
#pragma mark 请求数据
- (void)post{
//    [MBProgressHUD showHUDAddedTo:self animated:YES];
...
//                    task.dealLineTime       = [dic objectForKey:@"dealLineTime"];
